...
## 2026-07-25 19:05 ICT

**Track 1 — เนื้อหาเชิงวิชาการ** · บัญชีที่ถือ A/B (green/EGAT/financial)

ต่อจากเอนทรี 17:50 ของกะเดียวกัน — ปิดหนี้ที่ค้างไว้ + ทำงาน B จนจบ

### สิ่งที่ทำเสร็จแล้ว (Completed Tasks)

**`23a47d7` — งาน B: P50/P90 + Monte Carlo ใน `financial/` (ตาม WORK_SPLIT)**
`financial/uncertainty.py` ใหม่ แยกความไม่แน่นอนเป็น 2 เรื่องโดยเจตนา ไม่รวมเป็นแถบเดียว:
(1) **P50/P90 ของพลังงาน** = ความผันผวนของ*แดด*แต่ละปี (2) **Monte Carlo** สุ่ม
CAPEX/ค่าไฟ/WACC/พลังงานพร้อมกัน = ความไม่แน่ใจใน*สมมติฐาน* บนโครงการนี้แถบที่ 2
กว้างกว่าแถบแรกมาก และการแยกให้เห็นคือประเด็นทั้งหมด
ผลจริง (GIS+ISB 200.2 kWp): P50 377,081 → P90 357,751 kWh · NPV 7.50–10.79–14.84 ล้าน ·
payback 3.22–4.02–4.89 ปี · โอกาสขาดทุน 0%

**`a84386a` — แก้ CI แดง #169/#170**

### บริบทและสถานะปัจจุบัน (Current Context & State)

- **branch** HEAD `a84386a` · CI #172 กำลังรัน (#169/#170 แดงและแก้แล้ว)
- **ผลรัน**: web **523 passed / 55 files** · financial **58** · api **355** ·
  lint/build/tsc/ruff clean

**⚠️ กติกาที่เพิ่งเจ็บมา — ด่านฝั่ง web มี 4 ตัว ไม่มีตัวไหนครอบตัวอื่น**
conflict ครั้งเดียวจากการ rebase ทำให้เกิดบั๊ก 2 ตัว ตัวที่ 2 คือวงเล็บปิด `}` ของ
`interface GridCarbonResponse` หายไป → `types.ts` **parse ไม่ผ่าน** แต่
`vitest` 523 ตัวผ่าน · `vite build` ผ่าน · `tsc --noEmit` **เงียบสนิท**
มีแค่ `npm run lint` (oxlint) ที่จับได้ และผมข้ามด่านนั้นไป → CI แดง 2 รอบ
**ก่อน push ต้องรันครบ: `npm run lint` + `vitest` + `vite build` + `tsc --noEmit`**

**หนี้ live-verify — ปิดแล้วทั้งหมด** (ยิงเบราว์เซอร์จริงรอบนี้):
- `FinancialUncertaintyPanel`: ขึ้นครบ · ทิศทางถูกต้องในของจริง —
  `ระยะคืนทุน` กรณีแย่ = **4.89 ปี** (ตัวมาก) ส่วน `NPV` กรณีแย่ = **7.50 ล้าน** (ตัวน้อย)
- `OfficialSourcesPanel`: 5 การ์ด เขียวทุกหน่วยงาน · คำเตือน GEF ขึ้นจริง
- mobile h-overflow **0px ทั้งสองหน้า** · ไม่มี console error

**gotcha จากการ verify**: หน้า `/financial` ใช้เวลา **~6.4 วินาที** กว่าผลจะขึ้น
(warm) แต่ **request แรกหลัง API บูตช้ากว่า 14 วินาที** เพราะ pvlib warm-up —
เทส Playwright ที่รอแบบตายตัวจะ flaky ให้ใช้ `waitFor` แทน `waitForTimeout`
Monte Carlo **ไม่ได้ทำให้ช้าเพิ่ม**: panel ขึ้นพร้อม KPI card วินาทีเดียวกัน

**ประสิทธิภาพ**: `compute_financial_analysis` เร็วขึ้น **50 เท่า** (25.4s → 0.51s
ต่อ 2,000 trials) เพราะเดิมสร้าง pandas Series 1 ตัวต่อปีต่อ trial ทั้งที่
degradation เป็นเชิงเส้น — มีเทสล็อกกับ `apply_scenario` ตลอด 25 ปีแล้ว

### เป้าหมายและงานต่อไป (Next Steps for the Next Session)

1. **เช็ค CI #172 (`a84386a`)** ให้เขียวก่อน
2. **ขอตัวเลขจาก user**: ค่าความไม่แน่นอนของ CAPEX (ตอนนี้ ±฿4,000/kWp) และ
   WACC (±1.5%) เป็นค่าที่**ผมเดา** — เป็นตัวกำหนดความกว้างของแถบ NPV ทั้งหมด
   ถ้าได้ราคา EPC จริงแล้วตั้ง CAPEX spread เป็น 0 ได้เลย (แก้ในหน้า Settings ไม่ต้อง deploy)
   ⚠️ **อย่าเสนอ benchmark ราคาตลาดซ้ำ** — user ปฏิเสธไปแล้ว
3. งานที่ยังไม่มีใครเริ่มและเป็นของ Track 1: **#152 ระบุแหล่งที่มาของอุณหภูมิรายชั่วโมง**
   (งานเล็ก) · **#151 ทำภาษาในเว็บให้เป็นไทยทั้งหมด** (งานใหญ่ ควรถาม user ก่อน)
4. งาน C/D (เงา ray-trace + ตัวหามุมที่ดีที่สุด) เป็นของอีกบัญชี — ไม่แตะ

## 2026-07-25 19:41 ICT

**Track 1 — เนื้อหาเชิงวิชาการ** · บัญชีที่ถือ A/B (green/EGAT/financial)

ต่อจากเอนทรี 19:05 ของกะเดียวกัน — user สั่ง "อะไรที่หาค่าจริงจากเว็ปไซต์ก็ดึงมาใส่"
กะนี้จึงเป็นการ **ไล่ลบค่าเดาทิ้ง แล้วเอาของจริงมาแทน** ไม่ใช่การเพิ่มฟีเจอร์

### สิ่งที่ทำเสร็จแล้ว (Completed Tasks)

**`c8cc728` — แก้ CI แดง #172/#173 (ของจริง) + ปิดงาน #152**
CI ยังแดงต่อจากรอบก่อน ผมเดาว่า "แก้ยังไม่ขึ้น" — **ผิด** พอไปดึง log จริงพบว่า
vitest ผ่าน 523/523 แต่ **`npm run build` พัง**: `FinancialPage.test.tsx` error
TS2322 เพราะ `tsc -b` **ตรวจไฟล์เทสด้วย** ส่วน `npx tsc --noEmit` **ไม่ตรวจ**
แก้โดยทำ `uncertainty` เป็น optional ใน `types.ts` → **CI #174 เขียว**
พร้อมกันนั้นปิด **#152**: หน้า Forecast ติดป้ายที่มาของทุกช่องในตาราง 9 ตัวแปร
(`GFS · 2 เมตร`, `GFS · 10 เมตร`, `GFS · SSRD`, `คำนวณเอง (pvlib)`) + เขียนตรงๆ
ว่า **"ไม่ใช่ค่าที่วัดด้วยเซนเซอร์หน้างาน"** + เอาอุณหภูมิของ EGAT มา cross-check

**`48f6459` — เลิกเดาความแปรปรวนรายปี ใช้ข้อมูลจริง 26 ปีแทน**
`financial.annual_yield_cv_pct` เคยตั้งไว้ **4.0%** = จุดกึ่งกลางของช่วง 3–5%
ที่งานวิจัยเขตร้อนรายงาน คือ**ค่าที่วัดจากที่อื่น** ค่านี้เป็นตัวกำหนด**ความกว้าง**
ของแถบ P50/P90 ที่เว็บเผยแพร่ จึงควรตรวจ ไม่ใช่รับมาใช้เฉยๆ
ดึง NASA POWER (`ALLSKY_SFC_SW_DWN`, ไม่ต้องใช้ key — แหล่งเดียวกับที่เว็บใช้ทำ UV อยู่แล้ว)
รายวัน ที่**พิกัดหนองแฟบเอง (12.71 N / 101.15 E)** รวมเป็นรายปี ครบ 26 ปี (2000–2025):

    เฉลี่ย 1,852.6 kWh/m²/ปี · SD 39.0 · **CV = 2.11%**
    ปีแย่สุด 2011 = 1,792.1 · ปีดีสุด 2004 = 1,939.1

**แดดที่นี่นิ่งกว่าที่งานวิจัยสมมติเกือบเท่าตัว** → แถบแคบลงครึ่งหนึ่ง:
ที่ 377,081 kWh/ปี ส่วนต่าง P50→P90 ลดจาก **~19,330 เหลือ ~10,197 kWh/ปี**
ของเดิมบอก downside แย่เกินจริง
- `financial/interannual.py` — ฟังก์ชันบริสุทธิ์ `annual_totals`/`annual_cv_pct`
  ทิ้ง fill value (−999) ก่อน แล้วเก็บ**เฉพาะปีที่ครบปฏิทิน** (ปีที่ขาดเดือนฝน
  จะรวมได้ต่ำแล้วอ่านเป็น "ปีแย่" ทั้งที่ไม่ใช่ = ปั้นความแปรปรวนขึ้นมาเอง)
  ปีครบ < 2 ปี คืน `None` ไม่ใช่ 0% — "มีข้อมูลปีเดียว" กับ "ที่นี่ไม่เคยแปรปรวน"
  เป็นคนละเรื่อง
- `financial/scripts/derive_annual_cv.py` — คำนวณซ้ำกับ API จริงได้ ตรวจสอบย้อนได้
- **origin ใหม่ `derived`** ป้ายเขียวน้ำทะเล — จงใจไม่ให้เหมือน `confirmed`
  (ไม่มีใครในโครงการเซ็นรับรอง) และไม่ใช่ `literature` (ซึ่งแปลว่าวัดที่อื่น)
- step 0.5 → 0.01 ไม่งั้นกดค่า 2.11 จากฟอร์มไม่ได้
- **ข้อจำกัดที่เขียนไว้บนหน้าจอ ไม่ได้ซ่อน**: นี่คือความแปรปรวนของ**แสง** ใช้แทน
  ความแปรปรวนของ**ไฟที่ผลิตได้** — ทั้งปีมันวิ่งตามกันใกล้มากเมื่อเฉลี่ยอุณหภูมิ
  กับฝุ่นแล้ว นี่คือเหตุผลที่เป็น `derived` ไม่ใช่ `confirmed`

**`d97b8d9` — เจอสองหน้าตีราคาไฟหน่วยเดียวกันไม่เท่ากัน**
`/financial` ใช้ **4.00 บาท/kWh** (เลขกลม ในโค้ดเขียนเองว่า "ต้องไปยืนยันกับ
โครงสร้างค่าไฟจริง") ส่วน `/savings` ใช้ **4.1025** = อัตราจริงจากประกาศประเภทที่ 4
TOU Peak ระดับ HV ที่ user ยืนยันไว้ตั้งแต่ 2026-07-19 → **การยืนยันนั้นเกิดขึ้นแล้ว
แต่ไปไม่ถึงหน้า Financial** ใครเอา payback หน้าหนึ่งไปเทียบเงินประหยัดอีกหน้าจะไม่ลงกัน
ปรับให้ตรงกันที่ 4.1025 + เทสล็อกกับ `green_savings.NORMAL_RATE_THB_PER_KWH`
ไม่ให้แยกกันอีก · step 0.05 → 0.0001 (0.05 ลงไม่ถึงทศนิยม 4 ตำแหน่ง)

**ตัวเลขที่เผยแพร่ขยับ (แจ้ง user แล้วในแชท พร้อมเสนอย้อนกลับ):**

| | เดิม 4.00 | ใหม่ 4.1025 |
|---|---|---|
| ระยะคืนทุน | 5.028 ปี | **4.901 ปี** |
| NPV | 9,145,758 | **9,600,377** |
| IRR | 19.98% | **20.53%** |
| LCOE | 2.2720 | 2.2720 (ไม่ขยับ — เป็นต้นทุน) |

### บริบทและสถานะปัจจุบัน (Current Context & State)

- **branch** HEAD `d97b8d9` · CI **#174 เขียว** (ตัวหลัง 2 commit กำลังรัน)
- **ผลรัน**: web **528 passed / 55 files** + lint + build · api **360** ·
  financial **72** · ruff clean
- **⚠️ ด่านฝั่ง web มี 3 ตัว ไม่มีตัวไหนครอบตัวอื่น — ต้องรันครบก่อน push**
  `npm run lint` (oxlint — จับ syntax ที่อีกสองตัวปล่อยผ่าน) +
  `npx vitest run` + **`npm run build`** (= `tsc -b && vite build`;
  **`tsc -b` ตรวจไฟล์เทส แต่ `npx tsc --noEmit` ไม่ตรวจ** — นี่คือตัวที่ทำ CI
  แดง 2 รอบล่าสุด `tsc --noEmit` ไม่ใช่ด่านที่เชื่อได้)
- **ระบบ origin ตอนนี้มี 6 แบบ**: `confirmed` / `as-built` / `derived` (ใหม่) /
  `literature` / `placeholder` / `tuning` — ถ้าเพิ่มอีก ต้องใส่ใน `ORIGIN_LABELS`
  ด้วย (มีเทสบังคับแล้ว) และเพิ่มสีใน `SettingsPage.css`
- **ที่ตรวจแล้วตั้งใจไม่แตะ**: ค่าคงที่ soiling (PM10 อ้างอิง 50 µg/m³ ฯลฯ) —
  มันคือ**ตัวหารสำหรับสเกล**ที่จับคู่กับค่า rate ไม่ใช่ค่าจริงของพื้นที่ ตัวที่เป็น
  ข้อมูลจริงคือ PM10/ฝน/ลม ที่วัดมาและถูกคูณเข้าไปอยู่แล้ว เปลี่ยนตัวอ้างอิงจะทำให้
  โมเดล**เพี้ยน ไม่ใช่แม่นขึ้น** · gridmix/EF/UGT ดึงของจริงจาก EGAT/กกพ. ไปแล้ว
- **CAPEX ฿30,000/kWp + WACC 8% ยังเป็นค่าเดา และหาจากเว็บไม่ได้** ต้องเป็นเลข
  ของโครงการเอง (สัญญา EPC จริง / ต้นทุนเงินทุนจริงของบริษัท)
  ⚠️ **อย่าเสนอ benchmark ราคาตลาดซ้ำ** — เสนอไปแล้วและ user ปฏิเสธแล้ว
  ค่า spread ที่ผมเดาไว้ (±฿4,000/kWp, ±1.5%) ก็ยังเป็นตัวกำหนดความกว้างแถบ NPV ทั้งหมด

### เป้าหมายและงานต่อไป (Next Steps for the Next Session)

1. **เช็ค CI ของ `d97b8d9`** ให้เขียวก่อนทำอย่างอื่น
2. ถ้า user บอกว่าไม่อยากให้ payback ขยับ → ย้อน `d97b8d9` เฉพาะค่า default
   (แก้ในหน้า Settings ได้เลย ไม่ต้อง deploy) ส่วน `48f6459` แยกจากกัน ย้อนคนละตัวได้
3. งาน Track 1 ที่ยังไม่มีใครเริ่ม: **#151 ทำภาษาในเว็บให้เป็นไทยทั้งหมด**
   (งานใหญ่ + ชนกับงาน `web/` ของอีกบัญชี — **ถาม user ก่อน** ถามไปแล้วยังไม่ตอบ) ·
   **#164 / #167** (3D View) น่าจะเป็นของ Track 1 แต่ไฟล์อยู่ใน `web/` ควรเช็คก่อน
4. งาน C/D (เงา ray-trace + ตัวหามุมที่ดีที่สุด) เป็นของอีกบัญชี — **ไม่แตะ**
   และห้ามแตะ `financial/`, `grid_carbon.py`, `green_savings.py` ฝั่งนั้นเช่นกัน

## 2026-07-25 23:30 ICT

**Track 1 — เนื้อหาเชิงวิชาการ** · บัญชีที่ถือ green/EGAT/financial

กะนี้ปิด **นวัตกรรมครบทั้งชุด** — POA ทั้งเว็บ + F + H + G (A/B ปิดไปก่อนหน้า)

### สิ่งที่ทำเสร็จแล้ว (Completed Tasks)

**0. แก้ปมชี้กันไปมา** — `WORK_SPLIT.md` เคยเขียนว่า C/D เป็นของ "อีกบัญชี" และ
HANDOFF อีกฝั่งก็เขียนแบบเดียวกัน → **ไม่มีใครเริ่ม** เลิกใช้คำว่า "อีกบัญชี"
เปลี่ยนไประบุตามสิ่งที่แต่ละฝั่งถือครองจริง

**1. `d691fb5` — POA ทั้งเว็บ (งานฐาน)**
ทุกแหล่งข้อมูลให้ GHI (แสงบนระนาบ**แนวนอน**) และโมเดล PV กินเข้าไปตรงๆ →
**มุมเอียงไม่เคยมีผลกับพลังงานเลย** มีผลแค่กับเงาแถวบังแถว แผงเอียง 10° กับ
แผงนอนราบให้ผลผลิตเท่ากันเป๊ะ
- `features/poa.py`: Erbs แยก beam/diffuse → Hay-Davies ฉายลงระนาบแผง
- วางไว้**ใน `simulate_zone_baseline`** ไม่ใช่ที่ผู้เรียก → call site ทั้ง 7 จุดได้ฟรี
- **377,081 → 385,774 kWh/ปี = +2.31%** (ค่าเดิมตรงกับที่รายงานไว้เป๊ะก่อนแก้
  = ยืนยันว่าส่วนต่างมาจาก transposition ไม่ใช่ค่าเลื่อน)
- เทส `ws_live` พังด้วยเหตุผลถูก: กรกฎาคมที่ 12.7°N ดวงอาทิตย์ผ่าน**เหนือ**ศีรษะ
  แผงเอียงลงใต้จึงหันหนีเล็กน้อยตอนเที่ยง ได้ 49.3 ไม่ถึง clip 50 kW → ผ่อนเป็นช่วง

**2. `688f4c9` — F: DC:AC + clipping**
- **การตัดยอดแทบไม่มีเลย**: GIS ที่ 1.20 เสียแค่ **0.10% = 122 kWh/ปี** เพราะ
  หัก loss ~20% แล้วฝั่ง AC แทบไม่แตะพิกัด · Jetty 1.14 ไม่ตัดเลย
- **ISB DC:AC = 0.93 → มี 9.9 kWp ใส่เพิ่มได้ฟรี** (~2,056 kWh/kWp/ปี) ← ของที่ทำได้จริง
- จงใจไม่บอก "อัตราส่วนที่ดีที่สุด" เพราะต้องใช้ CAPEX จริง แสดง marginal แทน
- เร่ง 91s → 5.3s (โมเดลเป็นเชิงเส้นกับกำลังติดตั้ง คำนวณครั้งเดียวแล้วสเกล
  ใส่ clipping ซึ่งไม่เชิงเส้นทีหลัง)

**3. `58482b5` — H: bifacial**
แผงจริงเป็น Trina Vertex N **bifacial 80±5%** แต่ระบบคิดเป็นหน้าเดียวมาตลอด
- GIS +10.9% · ISB +15.3% · **Jetty +3.3%** (เหนือทะเล albedo 0.07)
- **แยกรายโซนสำคัญกว่าค่ารวม** — ค่าเดียวทั้งไซต์จะประเมิน Jetty เกินหลายเท่า มีเทสกัน
- ⚠️ **ไม่เอาไปบวกในตัวเลขที่เผยแพร่** เพราะขึ้นกับ albedo + ความสูงที่ไม่มีใครวัด

**4. `c129a6c` — G: TOU peak/off-peak**
- Peak = 09:00–22:00 จ.–ศ. เท่านั้น · **32.3% ของพลังงานทั้งปี (274,719 kWh)
  ตกนอกช่วง Peak** แต่เว็บตีเป็นอัตรา Peak หมด
- 2 เรื่องที่มองข้าม: เสาร์-อาทิตย์ทั้งวัน + **06:00–09:00 ของวันทำงาน**
- **ไม่ขยับตัวเลขใดๆ**: `green.offpeak_rate_thb_per_kwh` default = อัตรา Peak
  → blended เท่าเดิมเป๊ะ · panel เขียน "ยังบอกไม่ได้" ไม่ใช่ "+0.0%"
- วันหยุดไม่นับ (กฟภ. คิดเฉพาะรายการที่ประกาศ) → สัดส่วน Peak ที่เห็นเป็น**เพดานบน**

**ผลรันล่าสุด:** api **380** · simulation **114** · features **114** · financial **74** ·
web **558** · ruff/build/lint clean

### บริบทและสถานะปัจจุบัน (Current Context & State)

- settings **88 ค่า / 10 กลุ่ม** · เพิ่ม 1 entry = ได้ช่องกรอกอัตโนมัติ
- **มี 4 lru_cache** ที่ `settings_service.apply_effective_settings` ต้องเคลียร์:
  `annual_shading_loss_pct` · `_profile_for_day` · `optimise_zone` · `analyse_zone`
  **เพิ่ม cache ใหม่เมื่อไร ต้องไปต่อที่นั่นด้วย**
- หน้า /forecast ตอนนี้มี panel ใหม่ 4 ตัวเรียงกัน: GridCarbon → Orientation →
  DcAc → Bifacial → Tou
- gotcha เดิมยังอยู่: `npm run test` ไม่ typecheck (ต้อง `npm run build`) ·
  ห้าม annotate recharts formatter · รัน ruff เอง · pytest ทีละ package

### เป้าหมายและงานต่อไป (Next Steps for the Next Session)

1. **เช็ค CI** ของ `c129a6c` ให้เขียว
2. **ข้อมูลที่ขอแล้วยังไม่ได้** — แต่ละอย่างห่างจากใช้งานได้แค่ 1 setting:
   - **อัตรา Off-Peak** ประเภทที่ 4 HV จากประกาศ + รายการวันหยุด Off-Peak ของ กฟภ.
   - **albedo จริง + ความสูงติดตั้งจริง** → ปลดล็อกให้ bifacial บวกได้
   - **มุมเอียง/ทิศจริง** → เปลี่ยน /orientation จาก "ห่างจากค่าสมมติ" เป็นข้อค้นพบจริง
   - **CAPEX + WACC จริง** (⚠️ **อย่าเสนอ benchmark ตลาดซ้ำ** user ปฏิเสธแล้ว)
   - **พิกัด+ความสูงถัง LNG** → ปลดล็อกงาน C (เงา ray-trace) ที่ยังติดอยู่
3. **งาน C (เงา ray-trace)** ยังติดข้อมูล ดู `HANDOFF_PROJECT_C_SHADING.md`
4. **#151 ทำเว็บให้เป็นไทยทั้งหมด** — งานใหญ่ ยังไม่มีใครเริ่ม ควรถาม user ก่อน
5. Live-verify panel ใหม่ 4 ตัวของกะนี้ด้วย browser จริง

---

## 2026-07-25 23:40 ICT — Track 1 (เนื้อหาเชิงวิชาการ)

> **แจ้งอีกบัญชี: งาน I / J / K เป็นของคุณแล้ว** — user สั่งเมื่อ 2026-07-25 ว่า
> *"ทำงานค้าง ส่วนนี้ให้เพื่อนทำนะ แจ้งเพื่อนด้วย"* หลังถามเรื่อง
> powerflow / EMS / high voltage / PV recycling
> 📄 สเปกฉบับเต็ม: **`HANDOFF_PROJECTS_IJK.md`** (ตัวเลขในนั้นรันจริงมาแล้วทุกตัว)

### สิ่งที่ทำเสร็จแล้ว (Completed Tasks)

**สำรวจ codebase ให้จบก่อนส่งงาน** — ไม่ได้ส่งหัวข้อลอยๆ ให้เพื่อน แต่ตรวจจนรู้ว่า
แต่ละอันทำได้จริงแค่ไหน แล้วเขียนเป็นสเปกพร้อมตัวเลขจริง:

- **I. Demand Charge — ช่องโหว่ที่ใหญ่ที่สุดที่เหลืออยู่**
  grep `demand_charge` / `demand charge` / `ค่าความต้องการ` / `peak_demand` ทั่ว
  `api/src`, `financial/src`, `simulation/src`, `forecast/src`, `libs/`, `config/`
  → **เจอที่เดียวในทั้ง repo** คือ `web/src/lib/assistantContent.ts:80` ซึ่งเป็นที่ที่
  **AI assistant อธิบาย demand charge ให้ผู้ชมฟัง แต่ระบบไม่เคยคำนวณมันเลย**
  ค่าไฟรายใหญ่มีสองก้อน (฿/kWh + ฿/kW ของพีคเดือน) เว็บโมเดลแค่ก้อนแรก
- **J. PV Recycling — ข้อมูลครบ ทำได้เลย** 600 แผ่น (84+196+320) รุ่น Trina Vertex N
  TSM-NEG21C.20 dual glass 2384×1303×33 mm → กระจกรวม **1,863.8 m²**
  ขาดแค่น้ำหนัก/แผ่นจาก datasheet + องค์ประกอบวัสดุจากงานวิจัย
- **K. Power Flow — ติดครึ่งเดียว** `assets.yaml` มีแค่ `sld_available: true`
  ไม่มีหม้อแปลง/ฟีดเดอร์/บัสบาร์/impedance เลย → load flow เต็มรูปแบบทำไม่ได้
  **แต่เจอบั๊กจริงระหว่างทาง**: `loss_model.py` ใช้ `DEFAULT_DC_WIRING_PCT = 2.0`
  เท่ากันทุกโซนและ **ไม่มีเทอมค่าสูญเสียสาย AC เลย** ทั้งที่ `assets.yaml` เก็บระยะ
  interconnection จริงไว้ **1,130 / 1,680 / 2,816 m** และ design constraint อนุญาตให้
  AC ตกได้ถึง **5%** (มากกว่าค่าสูญเสียสายทั้งหมดที่โมเดลคิดอยู่ 2.5 เท่า)
  → **ผลผลิต Jetty ที่เผยแพร่อยู่อาจสูงเกินจริง**

**เอกสารที่แก้/สร้าง:** `HANDOFF_PROJECTS_IJK.md` (ใหม่) ·
`WORK_SPLIT.md` (ตารางใหม่ A-K + รายการข้อมูลที่ยังขอ user 6 ข้อ) ·
`PROMPT_FOR_OTHER_ACCOUNT.md` (เปลี่ยนงานเพื่อนจาก C→D เป็น I→J→K)

### บริบทและสถานะปัจจุบัน (Current Context & State)

**ตัวเลขไซต์ที่รันจริงในกะนี้ — ใช้ต่อได้เลยไม่ต้องรันซ้ำ:**

| ค่า | ตัวเลข |
|---|---|
| กำลังติดตั้ง | 429.0 kWp DC / 400 kW AC / **600 แผ่น** |
| ผลผลิตรายปี | **850,109 kWh** (Peak 575,390 · Off-Peak 274,719) |
| โหลดคลัง | 13,500 kW → **118,260,000 kWh/ปี** |
| ค่าไฟคลัง | ~300 MTHB/ปี → **฿2.5368/kWh** (ต่ำกว่าอัตรา Peak ฿4.1025 มาก) |
| **โซลาร์ = สัดส่วนของโหลด** | **0.72%** |

**0.72% คือกุญแจของงาน I**: 429 kWp เทียบโหลด 13,500 kW = ลดพีคได้มากสุด 3.2%
และคลัง LNG เดินเครื่อง 24/7 โหลดแบน → **พีคของเดือนอาจเกิดตอนกลางคืน**
ซึ่งแปลว่าโซลาร์ลด demand charge ไม่ได้เลย — ข้อค้นพบด้านการลงทุนที่ยังไม่มีใครตรวจ
⚠️ ไซต์ **ไม่มี load profile ของคลัง** → ห้ามปั้นกราฟโหลด ให้ตอบเป็น**เส้นความไว**แทน

- settings **88 ค่า / 10 กลุ่ม** · **4 lru_cache** ที่ต้องเคลียร์ยังเหมือนเดิม
- แบ่งงานตอนนี้: **Track 1** ถือ POA/D/F/H/G + panel ใหม่ 5 ตัวบน /forecast ·
  **อีกบัญชี** ถือ I/J/K + B(P50/P90) ที่ยังค้าง · **C ว่าง** แต่ติดพิกัดถัง LNG

### เป้าหมายและงานต่อไป (Next Steps for the Next Session)

1. **เช็ค CI** ของ `c129a6c` / `86524be` ให้เขียว (ตอนเขียนยัง in_progress)
2. **Live-verify panel ใหม่ 5 ตัว** บนเบราว์เซอร์จริง — หนี้ค้างจากกะที่แล้ว
   (GridCarbon → Orientation → DcAc → Bifacial → Tou)
3. **ข้อมูลที่ขอ user แล้วยังไม่ได้ — ตอนนี้ 6 ข้อ** แต่ละข้อห่างจากใช้งานได้แค่ 1 setting:
   อัตรา Off-Peak + วันหยุด กฟภ. · มุมเอียง/ทิศจริง · albedo + ความสูงจริง ·
   CAPEX + WACC (⚠️ **อย่าเสนอ benchmark ตลาดซ้ำ**) ·
   🆕 อัตรา demand charge + ประเภทอัตรา/แรงดันของคลัง (งาน I) ·
   🆕 ขนาดสาย+ชนิดตัวนำเส้น 1,130 m (งาน K) · พิกัด+ความสูงถัง LNG (งาน C)
4. **#151 ทำเว็บให้เป็นไทยทั้งหมด** — งานใหญ่ ยังไม่มีใครเริ่ม ควรถาม user ก่อน

---

## 2026-07-26 03:55 ICT — Track 1 (เนื้อหาเชิงวิชาการ)

> user สั่ง: *"เน้นนวัตกรรมใหม่ๆ ด้าน forecast day ahead กับ intraday ดีกว่า
> ให้มันมี block แสดงค่าตัวใหญ่ๆอะไรแบบนี้ก็ดีนะ"* แล้วเลือกนวัตกรรม **ครบทั้ง 4 ข้อ (A/B/C/D)**
> หลังได้รับคำเตือนเรื่องเครดิต — ทำครบแล้วทั้ง 4 + KPI block โดย **push ทีละชิ้นที่เทสเขียว**

### สิ่งที่ทำเสร็จแล้ว (Completed Tasks)

**KPI block** (`c360d42`) — `web/lib/forecastKpi.ts` + `components/ForecastKpiBlock.tsx`
เดิม `kpi-row` มี 4 ช่องแต่เป็น**สถานะปัจจุบันล้วน** ไม่พูดถึง forecast เลย ตัวใหม่บอก
พลังงานที่คาดว่าจะได้ · พีค+เวลา · ความกว้างแถบความเชื่อมั่น · ระยะเวลาที่จะผลิต
เปลี่ยนตามปุ่ม Day-ahead/Intra-day · **step หา kWh ใช้ค่ามัธยฐาน** (ถ้าใช้ค่าเฉลี่ย ช่องว่าง 6 ชม.
ช่องเดียวทำให้พลังงานพองเกินเท่าตัว) · "พรุ่งนี้" นับตามวันแบบไทย · ไม่มีแถบ = "—" ไม่ใช่ "±0"

**A — ตรวจแถบความเชื่อมั่นจริง** (`b599128`) แถบที่กราฟวาดคือ quantile 0.05/0.95 = สัญญาว่า
ครอบคลุม 90% **แต่ไม่เคยมีใครตรวจ** (`metrics.picp` เป็นค่าตอน train — กับดักเดียวกับที่
ทำให้ต้องสร้าง verification.py) → เพิ่ม coverage จริง + PINAW + **pinball loss** (proper
scoring rule: ขยายแถบเฉยๆ ไม่ช่วย ต่างจาก coverage) + พลาดไปทางไหน + แยกตาม lead
· โหมดสำรองฟิสิกส์ไม่มีแถบ → ตัดออก ไม่นับเป็น "พลาด" · แก้ที่ต้นทาง: `100*(0.95-0.05)`
= 89.99999999999999 ซึ่งจะโผล่บนหน้าจอ

**C — จำแนกสภาพฟ้า** (`d89e65b`) `forecast/sky_condition.py` ใช้ kt = GHI/GHI ท้องฟ้าใส
แยก RMSE/skill ตาม ฟ้าใส/มีเมฆ/ครึ้ม · **เกณฑ์ 0.7/0.3 เป็นนิยาม ไม่ใช่ค่าที่วัดได้**
จึงเปิดเป็น setting · ใช้ NWP รอบล่าสุดของชั่วโมงนั้น · กลางคืนไม่มีสภาพฟ้า (ตัดออก) ·
ชั่วโมงที่ระบุไม่ได้ **นับแยกและรายงาน** ไม่ยัดเข้า "ครึ้ม" · panel บอกชื่อสภาพฟ้าที่ยากที่สุดให้เลย

**B — Ramp forecast** (`69394fa`) `forecast/ramp.py` + `/forecast/{zone}/ramp`
ระบบเดิมตอบแค่ "เท่าไร" ไม่เคยตอบ "เร็วแค่ไหน" · severity เป็น % ของกำลังติดตั้ง/ชม.
(15 kW/h = 30% ของ GIS แต่ 7.5% ของ Jetty) · เตือนเฉพาะระดับปานกลางขึ้นไป
(เตือนทุกวัน = ไม่มีใครอ่าน) · ครึ่งหลังเป็น**สถิติจากค่าที่วัดจริง** บอกว่าร่วงแรงกี่ครั้ง/ชั่วโมงไหน
⚠️ **เขียนไว้ชัดว่าไม่ใช่สัญญาณให้สั่งการ** — ไซต์ไม่มีแบตเตอรี่/ไม่มีระบบสั่งลด

**D — Forecast evolution** (`5fecf1f`) **ข้อมูลไม่เคยมี**: `forecast_history` PK =
(zone,horizon,target_time) + REPLACE → คำตอบรอบเก่าถูกทับหายทุกครั้ง จึงเพิ่มตาราง
`forecast_evolution` (เพิ่ม issued_at เข้า PK, prune 14 วัน, ไม่มีอะไร serve จากตารางนี้)
· รายงาน **แกว่งมากสุด** แยกจาก **ปรับรวม** (180→90→175 ปรับรวมแค่ -5 แต่แกว่ง 90)
· `is_converging` = null ถ้าน้อยกว่า 3 รอบ · แก้บั๊ก: `+00:00` ใน query กลายเป็นช่องว่าง → เคย 500

### บริบทและสถานะปัจจุบัน (Current Context & State)

- **settings 92 ค่า / 10 กลุ่ม** (เพิ่ม sky_clear_kt, sky_overcast_kt, ramp_moderate_pct, ramp_steep_pct)
- **route ใหม่ 2 ตัว**: `/forecast/{zone}/ramp` · `/forecast/{zone}/evolution`
  ⚠️ ต้อง register **ก่อน** `routes_forecast` เสมอ (FastAPI จับตามลำดับ ไม่งั้น `{horizon}` กินไปหมด)
- **panel ใหม่ 3 ตัวบน /forecast**: ForecastKpiBlock (บนสุด) · RampPanel · ForecastEvolutionPanel
  รวมกับของกะที่แล้วเป็น 8 panel
- **ตาราง DB ใหม่**: `forecast_evolution` — เริ่มเก็บตอน deploy จึงว่างช่วงแรก **โดยตั้งใจ** และหน้าจอบอกไว้
- **ผลรันล่าสุด**: forecast **234** · api **399** · web **602** · ruff/lint/build clean
- 4 lru_cache เดิมยังเหมือนเดิม (ของรอบนี้ไม่มี cache ใหม่)

### เป้าหมายและงานต่อไป (Next Steps for the Next Session)

1. **เช็ค CI** ของ `5fecf1f` ให้เขียว
2. **Live-verify panel ใหม่ 3 ตัวของรอบนี้** บนเบราว์เซอร์จริง — RampPanel/EvolutionPanel
   จะขึ้นสถานะ "ยังไม่มีข้อมูล" ในเครื่องทดสอบ ซึ่งถูกต้อง ต้องดูของจริงบน Railway
3. **ข้อมูลที่ขอ user แล้วยังไม่ได้ (6 ข้อเดิม)** — อัตรา Off-Peak · มุมเอียง/ทิศจริง ·
   albedo+ความสูง · CAPEX+WACC (⚠️ **อย่าเสนอ benchmark ตลาดซ้ำ**) · อัตรา demand charge ·
   ขนาดสายเส้น 1,130 m · พิกัดถัง LNG
4. **งาน I/J/K ยังเป็นของอีกบัญชี** ดู `HANDOFF_PROJECTS_IJK.md` (ยังไม่มีใครเริ่ม)
5. **#151 ทำเว็บให้เป็นไทยทั้งหมด** — งานใหญ่ ยังไม่เริ่ม ควรถาม user ก่อน

---

## 2026-07-26 04:20 ICT — Track 1 (เนื้อหาเชิงวิชาการ)

> **แจ้ง Track 2: งาน L / M / N / O (Interface) เป็นของคุณแล้ว**
> user ถามหา *"นวัตกรรมล้ำๆ เกี่ยวๆกับ interface web แบบทึ่งๆว้าวๆ"* เลือกครบทั้ง 4 ข้อ
> แล้วสั่งว่า **"เขียนสเปกไว้ให้เพื่อทำ"** (ไม่ให้ Track 1 ข้ามมาทำเอง)
> 📄 สเปกฉบับเต็ม: **`HANDOFF_PROJECTS_LMNO_UI.md`**

### สิ่งที่ทำเสร็จแล้ว (Completed Tasks)

**สำรวจ frontend ให้จบก่อนเขียนสเปก** — ผลสำรวจทำให้ข้อ L เปลี่ยนสเปกไปจากที่ user จินตนาการ:

- **L. เงาบนแผง** — user อยากได้ "เงาเมฆจริงเคลื่อนผ่านแผง" **แต่ทำแบบซื่อสัตย์ไม่ได้**
  codebase เขียนบอกไว้เอง **สองที่อิสระจากกัน**: `CloudLayer` docstring ("*one site-wide opacity
  scalar + a motion vector, not a spatial raster ... not a per-panel shadow simulation ... no data
  source in this app currently supports that claim*") และ `routes_irradiance_map` ("*sub-2km
  spatial variation is still interpolated texture*") · raw tile ราย pixel อยู่ใน MinIO ที่ไม่ได้ต่อกับ Railway
  → **ของที่ทำได้และยังไม่มีใครทำ: เงาจากดวงอาทิตย์** ซึ่งเป็นของจริง 100% (มีตำแหน่งดวงอาทิตย์จริง
  + พิกัดมุมแผงที่รังวัดจริง) แต่ฉาก 3D **ไม่เปิด shadow map เลย** — `castShadow` มีแค่บนตัวมาสคอต
  (บรรทัด 1255/1260) พอเปิดแล้วเงาแถวบังแถวจะโผล่มาเอง **ตรงกับที่ `annual_shading_loss_pct`
  คำนวณเป็นตัวเลขอยู่แล้ว** = ครั้งแรกที่ตัวเลขกับภาพพูดเรื่องเดียวกัน
- **M. WebXR** — ทำได้ ฉาก r3f พร้อม เติม `@react-three/xr` ตัวเดียว
  ⚠️ **iPhone/Safari ไม่รองรับ WebXR AR** ต้องถาม user ก่อนว่าจะเดโมด้วยอุปกรณ์อะไร
- **N. เสียง** — **คุ้มที่สุดต่อแรง ไม่ต้องเพิ่ม dependency** เพราะมี 2 ใน 3 ชิ้นแล้ว:
  `tts.ts` (พูดออก) + `assistant.ts` `ASSISTANT_INTENTS` (เข้าใจคำถาม) เหลือแค่ `SpeechRecognition`
  ⚠️ Firefox ไม่รองรับ · ศัพท์อังกฤษปนไทยจะรู้จำเพี้ยน ต้อง normalize ก่อนเข้า intent matcher
- **O. Provenance Inspector** — วัตถุดิบครบแต่กระจัดกระจาย (`origin` ของ 92 settings ·
  `official_sources.py` · `soiling_source` · คอมเมนต์ `assets.yaml` · `MIX_ORIGIN_*` · `reference_note`)
  วิธีที่ถูกคือทำ provenance map ที่เครื่องอ่านได้ **ไม่ใช่ tooltip เขียนมือ** (พอค่าเปลี่ยน tooltip จะโกหก)
  · ก้ำกึ่งสองแทร็ก ต้องคุยขอบเขตก่อน

**เอกสาร:** `HANDOFF_PROJECTS_LMNO_UI.md` (ใหม่) · `WORK_SPLIT.md` (ตาราง A–O) ·
`PROMPT_FOR_OTHER_ACCOUNT.md` (เพื่อนมีงานค้าง 2 ชุดแล้ว: Interface L/M/N/O + เนื้อหา I/J/K)

### บริบทและสถานะปัจจุบัน (Current Context & State)

- **กฎใหม่ที่ควรจำ**: กฎห้ามปั้นข้อมูล**ใช้กับกราฟฟิกด้วย** — ภาพที่วาดรายละเอียดเกินกว่าที่ข้อมูล
  รองรับ (เงาเมฆเป็นรูปร่างจากตัวเลขเดียวทั้งไซต์) คือการปั้นข้อมูลเหมือนกัน แค่ปั้นด้วยพิกเซล
- ของที่ frontend มีอยู่แล้วและอย่าสร้างซ้ำ: ดวงจันทร์ข้างขึ้นข้างแรมถูกต้อง · ควบคุมด้วยมือ (MediaPipe) ·
  TTS · ผู้ช่วย AI + intent engine · แชท+presence+สติกเกอร์ · ภาพดาวเทียมปูพื้น · overlay ความเข้มแสง
- **dependency ฝั่ง web มีแค่ 9 ตัว** — โปรเจกต์นี้ประหยัด dep มาตลอด M ต้องเพิ่ม 1 ตัว, N ไม่ต้องเพิ่ม
- Track 1 รอบนี้ไม่ได้แตะโค้ดเลย (เขียนเอกสารอย่างเดียว) — โค้ดล่าสุดยังเป็น `3223554`

### เป้าหมายและงานต่อไป (Next Steps for the Next Session)

1. **Live-verify panel ใหม่ 3 ตัว** (KPI block / Ramp / Evolution) บนเบราว์เซอร์จริง — หนี้ค้างจากกะที่แล้ว
   Ramp/Evolution จะขึ้น "ยังไม่มีข้อมูล" ในเครื่องทดสอบ ต้องดูของจริงบน Railway
2. **เช็ค CI** ของ `5fecf1f` / `3223554`
...
## 2026-07-26 (รอบที่ 4) — Track 1 ทำงาน graphic design ครบ 4 ข้อ (P/Q/R/S)

> ⚠️ **Track 1 (เนื้อหาเชิงวิชาการ) ทำงาน Track 2 อีกรอบ** — user เลือกเองทั้ง 4 ข้อ
> และตอบว่า "ให้บัญชีนี้ทำเลย" · **เพื่อนอย่าทำซ้ำ**

### สิ่งที่ทำเสร็จแล้ว (Completed Tasks)

**P. ระบบตัวอักษรไทย** (`d25ce2d`) — รอบนี้เป็นการ "ซ่อม" มากกว่า "ออกแบบ" เพราะสำรวจแล้วเจอบั๊กจริง
- `<html lang="en">` ทั้งที่เว็บเป็นไทย → เบราว์เซอร์ตัดบรรทัดไทยผิด เลือกฟอนต์ fallback ผิด
  screen reader อ่านไทยด้วยเสียงอังกฤษ · แก้เป็น `lang="th"` + เพิ่ม meta description ที่ไม่เคยมี
- **ไม่มีฟอนต์ไทยเลย** — `--sans` มีแต่ฟอนต์ละติน ไทยเลยตกไปใช้ของ OS (Windows ได้ Leelawadee,
  Mac ได้ Thonburi) = ทุกเครื่องเห็นคนละแบบ และคนละแบบกับละตินในบรรทัดเดียวกัน
  → self-host **IBM Plex Sans Thai** 4 น้ำหนัก แยก subset ไทย/ละติน **รวม 118 KB** preload 4 ไฟล์
  · ไม่ใช้ CDN (โปรเจกต์นี้ไม่โหลดอะไรจาก third party ตอน runtime + firewall ในไซต์ PTT)
  · OFL 1.1 → มี `public/fonts/OFL.txt` ตามเงื่อนไขลิขสิทธิ์
- **line-height เป็นของละติน** — ไทยซ้อน 4 ชั้น (สระล่าง/ตัวอักษร/สระบน/วรรณยุกต์) แต่ body เป็น 145%
  และ h2 เป็น **118% ซึ่งวรรณยุกต์โดนตัดหัว** → `--lh-body: 1.65` `--lh-heading: 1.3` `--lh-compact: 1.5`
- **ตัด letter-spacing ทิ้งหมด** รวมทั้งค่าติดลบบน h1/h2 (ดึงวรรณยุกต์ออกจากตัวอักษร)
- **ตัด italic ทิ้ง 4 จุด — เกือบเป็น regression** โปรเจกต์ตั้ง `font-synthesis: none` และไม่มี italic face
  ถ้าไม่แก้ ทั้ง 4 จุดจะแสดงเป็นตัวตรงเงียบๆ · และไทยไม่มี italic อยู่แล้ว ใช้ opacity/ขนาดแทน

**Q. รายงาน PDF ที่ทุกตัวเลขมีที่มา** (`91322ae`) — เดิม**ไม่มี `@media print` แม้แต่บรรทัดเดียว**
- `src/print.css` + `PrintReport.tsx` · ปุ่มอยู่ที่หน้า Energy Report และ Financial
- **ของเด็ดคือภาคผนวก**: บนจอคลิก ⓘ ได้ แต่บนกระดาษไม่มี popup ตัวเลขเลยกลายเป็น "ข้อเท็จจริง"
  → หน้าสุดท้ายพิมพ์สายที่มาของทุกตัวเลขในหน้านั้น **เรียงจากอ่อนที่สุดก่อน** สร้างจาก registry ตัวเดียวกับ ⓘ
  · ถ้าดึงสายไหนไม่ได้ **บอกว่าไม่ครบ** (ภาคผนวกที่แอบตกตัวเลขแย่กว่าไม่มีเลย)
  · banner คำเตือนของหน้า Financial **จงใจไม่ใส่ `.print-hide`**
- **ต้อง override token ครบทั้ง 37 ตัว ไม่ใช่ 8 ตัว** — รอบแรกทำแค่ 8 แล้วพิมพ์ PDF จริงในโหมดมืดออกมา
  เห็น `<code>` เป็นตัวดำบนพื้นดำ และสีกราฟทั้ง 23 เส้นจะเป็นเวอร์ชันสำหรับพื้นมืด (จางบนกระดาษขาว)

**R. โหมดนำเสนอ + ระบบ motion** (`4427b73`)
- `ThemeToggle` วนสี่สถานะ ตามระบบ → สว่าง → มืด → **นำเสนอ** (คอนทราสต์สูง ตัวอักษรฐาน 22px
  เส้นกราฟหนา 3.2 สีกราฟเข้มสุดที่ใช้ได้ — สำหรับโปรเจกเตอร์ในห้องมีแสง)
- **palette ย้ายมาอยู่ที่เดียว** — จาก media query มาเป็น `:root[data-theme='dark']`
  และ `index.html` stamp `data-theme` แบบ synchronous ก่อน paint แรก (ไม่มีไฟกระพริบ)
  · ทำให้ rule ที่ตายอยู่ใน `EnergySavingsTable.css` (`:root[data-theme='dark']` ที่ไม่เคยมีอะไร set) มีชีวิต
- `useCountUp` — **ไม่วิ่งตอน mount** (วิ่งจาก 0 = โชว์ตัวเลขที่โรงไฟฟ้าไม่เคยผลิต) และ**ลงพอดีเป๊ะ**
- **บั๊กที่ทำเองแล้วจับได้**: `ThemeToggle` เรียก `matchMedia` โดยไม่ guard → jsdom ไม่มี →
  Layout test แดง 6 ตัว และของจริงจะพัง app shell ทั้งอัน แก้ที่ต้นทางด้วย `prefersDarkNow()`

**S. โปสเตอร์ generative** (`04eec5b`) — `GET /poster/{zone}` + `YearPoster.tsx`
- วงกลม 365 เส้น เส้นละวัน ยาวจากพระอาทิตย์ขึ้นถึงตก · เดือนละสี · ดาวน์โหลดเป็น SVG
- **ข้อจำกัดด้านความซื่อสัตย์คือสิ่งที่กำหนดรูปแบบ**: โปสเตอร์ที่ "ควรจะเป็น" คือตาราง 365×24 ช่อง
  แต่โมเดลพลังงานรายปีของโปรเจกต์นี้**คือวันตัวแทน 12 วัน** → ระบายสี 8,760 ช่องคือการปั้นค่า 8,748 ค่า
  → **แยกสองความละเอียดชัดเจน**: รูปทรง = รายวัน แม่นยำ (pvlib ที่พิกัดจริง 12.71°N)
  · สี = รายเดือน เป็นแบบจำลอง · **API จงใจไม่มีฟิลด์พลังงานรายวัน** และมีเทสต์ยืนยันว่าไม่มี
- สีวัดจาก 0 ถึงเดือนที่ดีที่สุด **ไม่ยืด min-max** เพราะเดือนต่างกันแค่ 8.4k–11.2k kWh
  การยืดจะทำให้ส่วนต่าง 25% ดูเหมือน 100% · ภาพที่ออกมาค่อนข้างเรียบ = ข้อค้นพบจริงของไซต์นี้

### บริบทและสถานะปัจจุบัน (Current Context & State)

- ตัวเลขล่าสุด: **web 678 tests / 73 ไฟล์** · **api 416 tests** · settings 92 ค่า / 10 กลุ่ม · forecast 234
- **วิธีตรวจงานดีไซน์ที่ใช้รอบนี้ และควรใช้ต่อ**: อย่าอ่าน CSS แล้วเชื่อ — เรนเดอร์ผ่าน headless
  Chromium (`executablePath: '/opt/pw-browsers/chromium-1194/chrome-linux/chrome'` เพราะ playwright
  ใน repo คาด build 1228 แต่เครื่องมี 1194) แล้ว**ดูภาพจริง** · รอบนี้จับได้ 3 บั๊กด้วยวิธีนี้
  (code บนพื้นมืดตอนพิมพ์ · ธีมทั้ง 3 ที่ harness แรกผิดเพราะ `:root` แมตช์แค่ `<html>` · ป้ายชั่วโมงทับกัน)
- **`.print-hide`** = คลาสสำหรับซ่อนตอนพิมพ์ · **`.print-only`** = แสดงเฉพาะตอนพิมพ์
- ถ้าเพิ่ม token สีใหม่ ต้องเพิ่มใน 3 ที่: `:root` (light) · `[data-theme='dark']` · `[data-theme='present']`
  และ `print.css` — มีสคริปต์เทียบ dark กับ print ใน commit ของ Q
- ยังเป็นแบรนช์ `claude/solar-optimization-forecasting-jryux7` เหมือนเดิม

### เป้าหมายและงานต่อไป (Next Steps for the Next Session)

1. **เช็ค CI** ของ `d25ce2d` / `91322ae` / `4427b73` / `04eec5b`
2. **Live-verify บน Railway จริง** — ของใหม่ที่ยังไม่เคยเห็นบนของจริง: ⓘ Provenance · ปุ่มพิมพ์ PDF +
   ภาคผนวก · ปุ่มสลับธีม 4 สถานะ · โปสเตอร์ · และหนี้เก่า Ramp / Forecast Evolution
3. **ดูฟอนต์ไทยบนเครื่องจริง** (Windows/Mac/มือถือ) ว่า Plex Sans Thai โหลดและอ่านง่ายจริง
4. **ถาม user**: (ก) จะเลื่อน `green.normal_rate_thb_per_kwh` เป็น `confirmed` ไหม
   (ข) เดโม AR ใช้อุปกรณ์อะไร (iPhone ไม่ได้)
5. **ข้อมูลที่ยังขาด 6 ข้อเดิม** — CAPEX + WACC ยังเป็นค่าประมาณ **อย่าเสนอราคาตลาดซ้ำ**
6. **งานของเพื่อน อย่าทำซ้ำ**: I (demand charge) · J (PV recycling) · K (power flow) → `HANDOFF_PROJECTS_IJK.md`
7. **#151 ทำเว็บเป็นไทยทั้งหมด** — งานใหญ่ ยังไม่เริ่ม · ตอนนี้ฐานพร้อมแล้ว (lang="th" + ฟอนต์ไทย)
