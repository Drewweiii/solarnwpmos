...
async def run() -> None:
    ...
    scheduler.start()
    logger.info("nwp ingestion scheduler started (cycles=%s, source_mode=%s)", settings.gfs_cycles, settings.source_mode)

    # Run once immediately on startup so operators don't wait for the next cycle to see data.
    await job.run_once()
