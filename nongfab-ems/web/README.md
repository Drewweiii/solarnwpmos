...
### 2026-07-26 - iPhone and iPad get AR after all (project T, Track 1)

Project M probed WebXR per mode and rendered nothing where it was absent, which
is every iPhone and iPad. Asked to cover iOS, so: **AR Quick Look**, Apple's own
path - `<a rel="ar">` at a `.usdz`, opened by the system AR viewer. The USDZ is
generated by the API from the same surveyed geometry the 3D scene uses (see
`api/README.md`).

Three details decide whether this works at all, and each fails *silently* - the
link merely navigates to a downloaded file, which reads as "AR is broken on
iPhone":

1. **The anchor must contain exactly one child element, an `<img>`.** Apple's
   requirement. Any other content and Safari stops treating it as an AR link. So
   the button label is drawn *into* an inline SVG data-URI image rather than
   placed beside it, the caption sits outside the anchor, and a test asserts
   `anchor.children.length === 1` and that the child is an `IMG`.
2. **`model/vnd.usdz+zip`**, not `application/zip`.
3. **The token goes in the query string**, because Quick Look fetches the URL
   outside the page and cannot send a header.

Support is decided by `relList.supports('ar')` - Apple's documented feature test
- rather than a user-agent sniff, because it answers the question actually being
asked and self-corrects as browsers change. `isAppleTouchDevice()` exists only
to explain *why* AR is unavailable when the test fails, and it carries a
`MacIntel` + `maxTouchPoints` clause because since iPadOS 13 an iPad reports
itself as a Mac.

**A bug this round introduced and caught.** `DOMTokenList.supports()` is
specified to **throw** a `TypeError` - not return false - when the attribute has
no supported-tokens definition, which is the case for anchor `rel` in every
engine without Quick Look. The unguarded feature test threw under jsdom, and
would have taken the 3D page down on Firefox. Now wrapped, with a test that
asserts it does not throw. Second time this shift a feature detection needed a
guard; the first was `matchMedia` in project R.

Gates: oxlint clean, 687 web tests across 74 files (+9), `tsc -b` + vite build
clean, api 425 (+9), ruff clean.
