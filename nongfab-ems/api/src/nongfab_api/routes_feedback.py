...
class FeedbackStore:
    ...
    async def add(self, username: str, role: str, text: str) -> FeedbackItem:
        async with self._session_factory() as session:
            row = FeedbackMessageORM(username=username, role=role, text=text, created_at=datetime.now(timezone.utc))
            session.add(row)
...
async def post_feedback(
    ...
    return await store.add(user.username, user.role, text[:MAX_TEXT_LENGTH])
